...
The bounded Linux smoke execution receipt is available as [execution-receipt-smoke.json](execution-receipt-smoke.json). A completed asynchronous HQ receipt for CB2 is available as [execution-receipt-hq-cb2.json](execution-receipt-hq-cb2.json); CB2 passed the 3-of-5 redocking gate and produced a ten-member Vina/Vinardo shortlist with seed dispersion. The consolidated [HQ study receipt](execution-receipt-hq-study.json) records all five target outcomes: CB2 and CYP19A1 passed, AR failed the validation gate, and ERbeta/HSD11B1 stopped during preparation. These receipts confirm real execution but do not override the partial full-study status.
