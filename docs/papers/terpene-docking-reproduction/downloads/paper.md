...
The bounded Linux smoke execution receipt is available as [execution-receipt-smoke.json](execution-receipt-smoke.json). It confirms real receptor preparation and Vina execution for one ligand/target, but it is not a full-panel reproduction and does not override the partial execution status.
